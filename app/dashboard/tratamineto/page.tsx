...
export default function Page() {
    ...
    const fetchTratamientos = async () => {
        setLoading(true);
        try {
            const response = await axios.get('http://localhost:3010/api/tratamientos');
            console.log(response);
            setTratamientos(response.data); // Store the fetched data in state
        } catch (err) {
    ...
    return (
        <div className="bg-gradient-to-r bg-neutral-800">
            <button type="button" className="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700" onClick={fetchTratamientos}>Get Tratamientos</button>
